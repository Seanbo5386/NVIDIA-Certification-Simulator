 ...
 */
export function getLinksByCategory(category: 'official' | 'additional'): DocumentationLink[] {
...
import { DOCUMENTATION_LINKS, DocumentationLink } from '../src/data/documentationLinks';
 ...
 */
function suggestFix(link: DocumentationLink, error: string): string | undefined {
  const { url, id } = link;

  // Common URL pattern fixes
  const suggestions: Record<string, string> = {
    // NVIDIA docs often move to versioned paths
    'docs.nvidia.com/datacenter/nvsm/': 'https://docs.nvidia.com/datacenter/nvsm/latest/',
    'docs.nvidia.com/deploy/xid-errors/': 'https://docs.nvidia.com/deploy/xid-errors/index.html',
...
test.describe('Documentation Links Validation', () => {
  ...
  test('all documentation links should be accessible', async () => {
    ...
    for (const link of DOCUMENTATION_LINKS) {
      ...
      if (!validation.ok) {
        result.suggestedFix = suggestFix(link, validation.error || `HTTP ${validation.status}`);
    ...
    expect(brokenLinks.length, `Found ${brokenLinks.length} broken link(s)`).toBe(0);
  });
  ...
  test('documentation page renders all resource links', async ({ page }) => {
    ...
    const officialLinks = DOCUMENTATION_LINKS.filter(l => l.category === 'official');
    ...
    const additionalLinks = DOCUMENTATION_LINKS.filter(l => l.category === 'additional');
