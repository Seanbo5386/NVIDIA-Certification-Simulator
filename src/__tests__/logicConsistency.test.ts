...
function runCommand(command: string, nodeId: string = 'dgx-00'): string {
  ...
  if (['ibstat', 'ibswitches', 'iblinkinfo', 'ibdiagnet', 'ib_write_bw', 'ib_read_bw'].includes(baseCmd)) {
    return simulators.infiniband.execute(parsed, context).output;
  }
  if (['mst', 'mlxconfig', 'mlxlink', 'mlxcables', 'mlxup', 'mlxfwmanager'].includes(baseCmd)) {
...
function getClusterMetrics() {
  ...
  const warningGpus = allGpus.filter((g) => g.healthStatus === 'Warning').length;
  const criticalGpus = allGpus.filter((g) => g.healthStatus === 'Critical').length;

  const totalPower = allGpus.reduce((sum, g) => sum + g.powerDraw, 0);
  const avgTemp = allGpus.reduce((sum, g) => sum + g.temperature, 0) / allGpus.length;
  const avgUtil = allGpus.reduce((sum, g) => sum + g.utilization, 0) / allGpus.length;

  const activeNVLinks = allGpus.reduce((sum, g) => sum + g.nvlinks.filter((l) => l.status === 'Active').length, 0);
...
describe('Category 1: Fault Injection Cascades', () => {
  ...
  describe('1.4 NVLink Failure', () => {
    ...
    it('should increment NVLink error counters', () => {
      const gpu = injectFault('dgx-00', 0, 'nvlink');
      const errorLinks = gpu.nvlinks.filter((l) => l.txErrors > 0 || l.rxErrors > 0);
...
describe('Category 2: Command Output Consistency', () => {
  ...
  describe('2.4 All Simulators Produce Valid Output', () => {
    const testCases = [
      ...
      { name: 'nv-fabricmanager query nvlink', cmd: 'nv-fabricmanager query nvlink' },
      ...
      { name: 'ipmitool dcmi power reading', cmd: 'ipmitool dcmi power reading' },
...
describe('Category 4: Cluster Aggregation Accuracy', () => {
  ...
  describe('4.2 NVLink Count Accuracy', () => {
    ...
    it('should decrease active count after NVLink fault', () => {
      ...
      const metricsAfter = getClusterMetrics();
      expect(metricsAfter.activeNVLinks).toBeLessThan(metricsBefore.activeNVLinks);
    });
  });
