...
export const DOMAIN_INFO: Record<DomainId, DomainInfo> = {
  domain1: {
    ...
    weight: 31,
    description:
      "Initialize and configure DGX systems from bare metal to operational state",
    objectives: [
  ...
  domain1: [
    {
      ...
      examTip:
        "Know the difference between -L (list GPUs) and -q (detailed query)",
    },
    {
      command: "ipmitool",
    ...
    },
    {
      ...
      examTip:
        "Know key NVIDIA services: nvidia-persistenced, nvidia-fabricmanager",
    },
  ],
  domain2: [
    {
    ...
    },
    {
      ...
      commonFlags: ["-i"],
      examTip:
        "Power limits must be within supported range (use -q for limits)",
    },
    {
...
export const EXAM_TIPS: ExamTip[] = [
  // General tips
  {
    ...
    description:
      "Domain 4 (Validation) at 33% and Domain 1 (Bring-Up) at 31% make up 64% of the exam.",
    details:
      "Focus your study time proportionally: Domain 4 > Domain 1 > Domain 3 > Domain 5 > Domain 2",
  },
  {
    id: "tip-2",
    category: "gotcha",
    ...
    description:
      "Questions may show command output and ask which command produced it.",
    details:
      "Memorize the distinctive output format of key commands like nvidia-smi, dcgmi, ibstat",
  },
  ...
  // Domain 1 tips
  {
    ...
    title: "POST Sequence",
    description:
      "Know the correct order: BIOS/UEFI → BMC init → Driver load → GPU enumeration",
  ...
  },
  {
    ...
    description:
      "ipmitool runs from host OS, but some operations (like sol activate) connect to BMC",
    details:
      "BMC has its own network interface configured separately from host",
  },
  {
    id: "tip-d1-3",
    category: "gotcha",
    ...
    description:
      "NVSwitch systems require nvidia-fabricmanager service for full NVLink functionality",
  ...
  // Domain 2 tips
  {
    ...
    description:
      "Enable MIG mode → Create GPU instances → Create compute instances",
    details:
      "nvidia-smi -mig 1 -i 0, then nvidia-smi mig -cgi <profile>, then nvidia-smi mig -cci",
  },
  {
    id: "tip-d2-2",
    category: "gotcha",
  ...
  },
  {
    ...
    description:
      "Know topology matrix abbreviations: NV# = NVLink, SYS = System/PCIe, PIX = Same switch",
  ...
  // Domain 3 tips
  {
    ...
    description:
      "Slurm GPU allocation: --gres=gpu:N or --gpus=N or --gpus-per-node=N",
    details:
      "Check gres.conf for GPU definitions: Name=gpu Type=a100 File=/dev/nvidia[0-7]",
  },
  {
    id: "tip-d3-2",
    category: "gotcha",
    ...
    description:
      "Docker needs --gpus flag, NVIDIA_VISIBLE_DEVICES env var, or device mapping",
  ...
  // Domain 4 tips
  {
    ...
    description:
      "Level 1: Quick (1-2 min), Level 2: Medium (5-10 min), Level 3: Long (15+ min)",
    details:
      "Level 3 includes stress tests - use for thorough validation, not daily checks",
  },
  {
    id: "tip-d4-2",
    category: "concept",
  ...
  },
  {
    ...
    description:
      "HPL efficiency depends on problem size - larger N = better efficiency",
  ...
  // Domain 5 tips
  {
    ...
    description:
      "XID 31: GPU memory page fault, XID 43: GPU stopped processing, XID 79: GPU fallen off bus",
  ...
  },
  {
    ...
    description:
      "Check: Fan RPM → Airflow → Ambient temp → GPU load → Power limits",
  ...
  },
  {
    ...
    description:
      "Volatile ECC errors clear on reboot, Aggregate ECC errors persist",
...
export const QUICK_REFERENCE: QuickRefSection[] = [
  ...
  },
  {
    ...
    items: [
      {
        label: "GPU persistence",
        value: "systemctl status nvidia-persistenced",
      },
      {
        label: "Fabric manager",
        value: "systemctl status nvidia-fabricmanager",
      },
...
export function getExamTips(domainId?: DomainId): ExamTip[] {
  ...
  return EXAM_TIPS.filter(
    (tip) => tip.domain === domainId || tip.domain === "general",
  );
}
...
export function getDocumentationLinks(domainId?: DomainId): DocLink[] {
  ...
  return DOCUMENTATION_LINKS.filter(
    (link) => !link.domain || link.domain === domainId,
  );
}
