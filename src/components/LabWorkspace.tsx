...
export function LabWorkspace({ onClose }: LabWorkspaceProps) {
  ...
  const revealNextHint = () => {
    if (hintEvaluation?.nextHint) {
      revealHintAction(activeScenario.id, currentStep.id, hintEvaluation.nextHint.id);
    }
  };
  ...
  return (
    <>
      ...
      <div className="bg-gray-800 px-6 py-4 border-b border-gray-700 flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-green-400">{activeScenario.title}</h2>
          <p className="text-sm text-gray-400 mt-1">
            {activeScenario.domain.toUpperCase()} • {activeScenario.difficulty} • {activeScenario.estimatedTime} min
          </p>
        </div>
        <button
          onClick={handleExit}
      ...
      <div className="bg-gray-800 px-6 py-3 border-b border-gray-700">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm text-gray-400">
            {Math.round(((currentStepIndex + 1) / activeScenario.steps.length) * 100)}% Complete
          </span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="mb-6">
          ...
          <div className="bg-gray-800 rounded-lg p-4 mb-4">
            <h4 className="text-sm font-semibold text-green-400 mb-2">OBJECTIVES</h4>
              ...
              {currentStep.objectives.map((objective, idx) => {
                ...
                return (
                  <li key={idx} className="text-sm text-gray-300 flex items-start gap-2">
                    ...
                    <span className={isPassed ? 'text-gray-400 line-through' : ''}>{objective}</span>
                  </li>
                );
              })}
            </ul>
          ...
          {validationConfig.enabled && currentValidation && !isStepCompleted && (
            <div className="bg-gray-800 rounded-lg p-4 mb-4 border-l-4 border-blue-500">
              <h4 className="text-sm font-semibold text-blue-400 mb-3">VALIDATION STATUS</h4>

              {/* Progress Bar */}
              <div className="mb-3">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-gray-400">Progress</span>
                  <span className="text-xs font-mono text-gray-300">{currentValidation.progress}%</span>
                </div>
                <div className="w-full bg-gray-700 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full transition-all duration-300 ${currentValidation.passed
                        ? 'bg-green-500'
              ...
              {currentValidation.feedback && (
                <div className={`text-sm p-2 rounded ${currentValidation.passed
              ...
              {/* Rule Status Details */}
              {currentValidation.ruleResults && currentValidation.ruleResults.length > 0 && (
          ...
          {currentStep.expectedCommands && currentStep.expectedCommands.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-4 mb-4">
              <h4 className="text-sm font-semibold text-blue-400 mb-2">
                SUGGESTED COMMANDS ({commandTracker.getExecutedCommands(currentStep.expectedCommands).length}/{currentStep.expectedCommands.length})
              </h4>
              <div className="space-y-2">
                {currentStep.expectedCommands.map((cmd, idx) => {
                  const isExecuted = commandTracker.getExecutedCommands([cmd]).length > 0;
                  return (
                    <div key={idx} className="flex items-start gap-2">
                      ...
                      <div className={`font-mono text-sm bg-black rounded px-3 py-2 flex-1 ${isExecuted ? 'text-green-400 border border-green-900' : 'text-gray-300'
                        }`}>
                        {cmd}
                      </div>
                    </div>
              ...
              {commandTracker.getExecutedCommands(currentStep.expectedCommands).length ===
                currentStep.expectedCommands.length && (
                  <div className="mt-3 text-xs text-green-400 flex items-center gap-2">
                    <Check className="w-4 h-4" />
                    All suggested commands executed! You can proceed to the next step.
                  </div>
                )}
              {commandTracker.getExecutedCommands(currentStep.expectedCommands).length > 0 &&
                commandTracker.getExecutedCommands(currentStep.expectedCommands).length <
                currentStep.expectedCommands.length && (
                  <div className="mt-3 text-xs text-yellow-400">
                    Try all suggested commands to fully understand this step before proceeding.
                  </div>
                )}
            </div>
          )}
            ...
            <div className="bg-gray-800 rounded-lg p-4 mb-4">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-sm font-semibold text-yellow-400 flex items-center gap-2">
                  ...
                  HINTS ({hintEvaluation?.revealedCount}/{hintEvaluation?.totalCount})
                </h4>
              ...
              {(hintEvaluation?.revealedCount || 0) > 0 && (
                <div className="space-y-3 mt-3">
                  {hintEvaluation?.allHints
                    .filter(hint => currentStepProgress?.revealedHintIds.includes(hint.id))
                    .map((hint, idx) => {
                      const levelEmoji = hint.level === 1 ? '💡' : hint.level === 2 ? '🔍' : '🎯';
                      return (
                        <div key={hint.id} className="bg-gray-900 rounded-lg p-3 border border-yellow-500/30">
                          <div className="flex items-start gap-2">
                            <span className="text-yellow-400 text-lg">{levelEmoji}</span>
                            <div className="flex-1">
                              <div className="text-xs text-gray-400 mb-1">
                                Hint {idx + 1} - Level {hint.level}
                              </div>
                              <p className="text-sm text-gray-200">{hint.message}</p>
                            </div>
                          </div>
                        </div>
                      );
              ...
              {(hintEvaluation?.revealedCount || 0) === 0 && (
                <div className="mt-3 text-sm text-gray-400 flex items-start gap-2">
                  ...
                  <div>
                    <p className="mb-2">Hints will unlock as you work through this step:</p>
                    <ul className="text-xs text-gray-500 space-y-1 ml-4">
                      <li>• After spending time on the step</li>
                      <li>• After failed validation attempts</li>
                      <li>• Based on commands you try</li>
                    </ul>
                    <p className="mt-2 text-yellow-400">💡 Tip: Type <span className="font-mono bg-black px-1 rounded">hint</span> in the terminal anytime!</p>
                  </div>
                </div>
              )}

              {/* No more hints available */}
              {!hintEvaluation?.nextHint && (hintEvaluation?.revealedCount || 0) > 0 && (hintEvaluation?.revealedCount || 0) < (hintEvaluation?.totalCount || 0) && (
                <p className="text-xs text-gray-500 mt-3">
                  ⏳ More hints will unlock as you continue working on this step.
                </p>
              )}

              {/* All hints revealed */}
              {(hintEvaluation?.revealedCount || 0) === (hintEvaluation?.totalCount || 0) && (hintEvaluation?.totalCount || 0) > 0 && (
            ...
            <div className="bg-gray-800 rounded-lg p-4 mb-4">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-sm font-semibold text-yellow-400 flex items-center gap-2">
                  ...
                  HINTS ({legacyCurrentHintCount}/{legacyAvailableHints.length})
                </h4>
              ...
              {legacyCurrentHintCount > 0 && (
                <div className="space-y-2 mt-3">
                  {legacyAvailableHints.slice(0, legacyCurrentHintCount).map((hint, idx) => (
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400">
            ...
            <span>Estimated time: ~{currentStep.estimatedDuration} minutes</span>
          </div>
        </div>

        {/* All Steps Overview */}
        <div className="mt-8 pt-6 border-t border-gray-700">
          <h4 className="text-sm font-semibold text-gray-400 mb-4">ALL STEPS</h4>
          <div className="space-y-2">
            {activeScenario.steps.map((step, idx) => {
              ...
              return (
                <div
                  key={step.id}
                  className={`flex items-center gap-3 p-3 rounded-lg ${isCurrent ? 'bg-gray-800 border border-green-500' : 'bg-gray-800/50'
                    }`}
                >
                  <div
                  ...
                  <span
                    className={`text-sm ${isCurrent ? 'text-white font-semibold' : isCompleted ? 'text-gray-400' : 'text-gray-500'
                      }`}
                  >
                    {step.title}
                  </span>
        ...
        <div className="mt-6 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-blue-300 mb-1">Use the Terminal</p>
              <p className="text-xs text-gray-400">
                Execute the commands shown above in the main terminal (on the right side of the screen) to complete this step.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="border-t border-gray-700 p-4 bg-gray-800">
                  ...
                  <div className="mt-2">
                    <div className="flex items-center gap-2 mb-1">
                      <div className="flex-1 bg-gray-700 rounded-full h-1.5">
                        <div
                          className={`h-1.5 rounded-full transition-all duration-300 ${currentValidation.progress > 70
                              ? 'bg-green-500'
                      ...
                      <span className="text-xs font-mono text-gray-400">{currentValidation.progress}%</span>
                    </div>
                    <div className="text-xs text-gray-400">
                      {currentValidation.matchedRules.length} of {currentValidation.matchedRules.length + currentValidation.failedRules.length} requirements met
                    </div>
                  </div>
                ) : (
        ...
        <button
          ...
          className={`w-full px-4 py-3 rounded font-semibold transition-colors flex items-center justify-center gap-2 ${isStepCompleted
