...
import { X, Clock, Flag, ChevronLeft, ChevronRight, CheckCircle } from 'lucide-react';
import type { ExamQuestion } from '@/types/scenarios';
import { loadExamQuestions, selectExamQuestions, calculateExamScore, ExamTimer, isExamPassed } from '@/utils/examEngine';
...
export function ExamWorkspace({ onClose }: ExamWorkspaceProps) {
  const { activeExam, startExam, submitExamAnswer, endExam, exitExam } = useSimulationStore();
  ...
  const handleToggleFlag = () => {
    ...
    const currentQuestion = questions[currentQuestionIdx];
    const newFlagged = new Set(activeExam.flaggedQuestions);
  ...
  const currentQuestion = questions[currentQuestionIdx];
  const userAnswer = activeExam?.answers.get(currentQuestion.id);
  const isAnswered = activeExam?.answeredQuestions.has(currentQuestion.id);
  const isFlagged = activeExam?.flaggedQuestions.has(currentQuestion.id);
    ...
    return (
      <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4">
        <div className="bg-gray-900 rounded-lg shadow-2xl w-full max-w-4xl border border-green-500 overflow-hidden">
          {/* Header */}
          <div className={`px-6 py-4 ${passed ? 'bg-green-600' : 'bg-red-600'}`}>
          ...
          <div className="p-6 max-h-[80vh] overflow-y-auto">
            ...
            <div className="bg-gray-800 rounded-lg p-6 mb-6">
              <h3 className="text-lg font-bold text-white mb-4">Performance by Domain</h3>
              <div className="space-y-4">
                {Object.values(breakdown.byDomain).map((domain) => {
                  const domainPassed = domain.percentage >= 70;
                  return (
                    <div key={domain.domainName} className="border-b border-gray-700 pb-4 last:border-0">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-white font-semibold">{domain.domainName}</span>
                      ...
                      <div className="flex items-center justify-between text-sm text-gray-400 mb-2">
                        <span>
                          {domain.questionsCorrect} / {domain.questionsTotal} correct
                        </span>
            ...
            {!passed && (
              <div className="bg-yellow-900 bg-opacity-30 border border-yellow-600 rounded-lg p-6 mb-6">
                <h3 className="text-lg font-bold text-yellow-400 mb-2">Areas for Improvement</h3>
            ...
            {Object.values(breakdown.byDomain).some(d => d.percentage < 70) && (
              <div className="bg-blue-900 bg-opacity-20 border border-blue-500 rounded-lg p-6 mb-6">
                <h3 className="text-lg font-bold text-blue-400 mb-3">Recommended Commands to Practice</h3>
                <p className="text-gray-300 mb-4">
                  Based on your performance, practice these commands in the simulator:
                </p>
                <div className="space-y-4">
                  {Object.values(breakdown.byDomain)
                    .filter(d => d.percentage < 70)
                    .map(domain => {
                      const domainCommands = getCommandRecommendations(domain.domainName);
                      return (
                        <div key={domain.domainName} className="border-l-4 border-blue-500 pl-4">
                          <h4 className="text-sm font-semibold text-blue-300 mb-2">{domain.domainName}</h4>
                          <div className="space-y-1">
                            {domainCommands.map((cmd, idx) => (
                              <div key={idx} className="font-mono text-sm text-gray-300 bg-black rounded px-3 py-1">
                                {cmd}
                              </div>
                            ))}
                          </div>
  ...
  if (showReview && activeExam?.breakdown) {
    ...
    const questionResult = activeExam.breakdown.questionResults.find(r => r.questionId === currentQuestion.id);
        ...
        <div className="bg-gray-900 rounded-lg shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col border border-green-500">
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="w-80 border-l border-gray-700 bg-gray-850 p-6 overflow-y-auto">
              <h4 className="text-sm font-semibold text-gray-400 mb-4">QUESTION NAVIGATION</h4>
              <div className="grid grid-cols-5 gap-2 mb-6">
                {questions.map((q, idx) => {
                  const qResult = activeExam.breakdown?.questionResults.find(r => r.questionId === q.id);
              ...
              <div className="bg-gray-800 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-gray-400 mb-2">SCORE SUMMARY</h4>
                <div className="text-3xl font-bold text-white mb-1">
                  {activeExam.breakdown.percentage}%
                </div>
                <div className="text-sm text-gray-400">
                  {activeExam.breakdown.earnedPoints} / {activeExam.breakdown.totalPoints} points
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div className="bg-gray-900 rounded-lg shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col border border-green-500">
        ...
        <div className="bg-gray-800 px-6 py-4 border-b border-gray-700 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-green-400">NCP-AII Practice Exam</h2>
            <p className="text-sm text-gray-400 mt-1">
              Question {currentQuestionIdx + 1} of {questions.length}
            </p>
          </div>
          <div className="flex items-center gap-4">
            <div className={`flex items-center gap-2 px-4 py-2 rounded ${timeRemaining < 300 ? 'bg-red-900 text-red-300' : 'bg-gray-700 text-gray-300'}`}>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-80 border-l border-gray-700 bg-gray-850 p-6 overflow-y-auto">
            ...
            <div className="grid grid-cols-5 gap-2 mb-6">
              {questions.map((q, idx) => {
                const answered = activeExam?.answeredQuestions.has(q.id);
                const flagged = activeExam?.flaggedQuestions.has(q.id);
                const current = idx === currentQuestionIdx;
            ...
            <div className="space-y-3 text-sm text-gray-400 mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>Answered ({activeExam?.answeredQuestions.size || 0})</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 bg-gray-700 rounded" />
                <span>Not Answered ({questions.length - (activeExam?.answeredQuestions.size || 0)})</span>
              </div>
            </div>
