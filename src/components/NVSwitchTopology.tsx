...
export const NVSwitchTopology: React.FC<NVSwitchTopologyProps> = ({
  ...
  const handleNodeClick = useCallback((gpuNode: GPUNode) => {
  ...
  useEffect(() => {
    ...
    gradient.append('stop').attr('offset', '100%').attr('stop-color', '#4a7c00');
    ...
    gpuNodes.forEach((gpuNode) => {
      nvSwitches.forEach((switchNode, switchIdx) => {
        ...
        const nvlink = gpuNode.gpu.nvlinks[switchIdx % gpuNode.gpu.nvlinks.length];
        links.push({
    ...
    gpuGroups
      ...
      .attr('fill', '#9CA3AF')
      .attr('font-size', '10px')
      .text((d) => `${Math.round(d.utilization)}% | ${Math.round(d.temperature)}°C`);
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
      ...
      {(selectedNode || hoveredNode) && (
        <div className="mt-4 p-3 bg-gray-900 rounded-lg border border-gray-700">
          <h4 className="text-sm font-semibold text-gray-300 mb-2">
            {selectedNode ? 'Selected' : 'Hovered'}: {(selectedNode || hoveredNode)?.type === 'gpu' ? (selectedNode || hoveredNode as GPUNode).name : ''}
          </h4>
          {(selectedNode || hoveredNode)?.type === 'gpu' && (
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <div className="text-gray-300">{((selectedNode || hoveredNode) as GPUNode).temperature}°C</div>
              <div className="text-gray-400">Utilization:</div>
              <div className="text-gray-300">{((selectedNode || hoveredNode) as GPUNode).utilization}%</div>
              <div className="text-gray-400">Power:</div>
              <div className="text-gray-300">
                {((selectedNode || hoveredNode) as GPUNode).gpu.powerDraw}W / {((selectedNode || hoveredNode) as GPUNode).gpu.powerLimit}W
              </div>
              <div className="text-gray-400">Memory:</div>
              <div className="text-gray-300">
                {Math.round(((selectedNode || hoveredNode) as GPUNode).gpu.memoryUsed / 1024)}GB / {Math.round(((selectedNode || hoveredNode) as GPUNode).gpu.memoryTotal / 1024)}GB
              </div>
              <div className="text-gray-400">NVLinks Active:</div>
              <div className="text-gray-300">
                {((selectedNode || hoveredNode) as GPUNode).gpu.nvlinks.filter(l => l.status === 'Active').length} / {((selectedNode || hoveredNode) as GPUNode).gpu.nvlinks.length}
              </div>
              <div className="text-gray-400">Health:</div>
              <div className={`font-semibold ${
                ((selectedNode || hoveredNode) as GPUNode).health === 'OK' ? 'text-green-400' :
                ((selectedNode || hoveredNode) as GPUNode).health === 'Warning' ? 'text-yellow-400' : 'text-red-400'
              }`}>
      ...
      <p className="mt-2 text-xs text-gray-500">
        Click on a GPU to select it. Green lines = active NVLinks. Dashed lines = inactive.
      </p>
    </div>
  );
};
