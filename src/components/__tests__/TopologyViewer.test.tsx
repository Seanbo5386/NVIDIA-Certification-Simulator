...
vi.mock('../InfiniBandTopology', () => ({
  InfiniBandTopology: ({ nodes, selectedNodeId, onNodeClick }: any) => (
    <div data-testid="infiniband-topology">
      ...
      <button onClick={() => onNodeClick?.('dgx-01')} data-testid="node-click">
        Click Node
      </button>
    </div>
  ),
...
describe('TopologyViewer', () => {
    ...
    gpus: [
      {
        ...
        migInstances: [],
        nvlinks: [
          { linkId: 0, status: 'Active', speed: 50, txErrors: 0, rxErrors: 0, replayErrors: 0 },
        ],
        healthStatus: 'OK',
        xidErrors: [],
  ...
  describe('View Mode Switching', () => {
    it('should render NVSwitch topology by default', () => {
      ...
      expect(screen.queryByTestId('infiniband-topology')).not.toBeInTheDocument();
    });
  ...
  describe('Fault Injection Panel', () => {
    ...
    it('should call updateGPU when thermal issue is injected', () => {
      ...
      expect(mockUpdateGPU).toHaveBeenCalledWith('dgx-00', 0, expect.objectContaining({
        temperature: 92,
    ...
    it('should call addXIDError when XID error is injected', () => {
      ...
      expect(mockAddXIDError).toHaveBeenCalledWith('dgx-00', 0, expect.objectContaining({
        code: 79,
...
describe('NVSwitchTopology Integration', () => {
  ...
  it('should pass node to NVSwitchTopology', () => {
    ...
    expect(screen.getByText('NVSwitch Topology for dgx-00')).toBeInTheDocument();
  });
});

describe('InfiniBandTopology Integration', () => {
  const mockNodes: DGXNode[] = Array.from({ length: 4 }, (_, i) => ({
    id: `dgx-0${i}`,
    ...
    hcas: [
      {
        ...
        caType: 'mlx5_0',
        firmwareVersion: '20.35.1012',
        ports: [{ portNumber: 1, state: 'Active' as const, physicalState: 'LinkUp' as const, rate: 400 as const, lid: i + 1, guid: `0x000000000000000${i}`, linkLayer: 'InfiniBand' as const, errors: { symbolErrors: 0, linkDowned: 0, portRcvErrors: 0, portXmitDiscards: 0, portXmitWait: 0 } }],
      },
    ],
    bmc: { ipAddress: `192.168.0.10${i}`, macAddress: `00:00:00:00:00:0${i}`, firmwareVersion: '1.2.3', manufacturer: 'NVIDIA', sensors: [], powerState: 'On' },
    cpuModel: 'AMD EPYC 7742',
    cpuCount: 128,
    ramTotal: 2048,
    ramUsed: 256,
  ...
  it('should pass all nodes to InfiniBandTopology', () => {
    render(<TopologyViewer nodes={mockNodes} selectedNodeId="dgx-00" />);

    fireEvent.click(screen.getByText('InfiniBand'));

    expect(screen.getByText('InfiniBand Topology - 4 nodes')).toBeInTheDocument();
  });

  it('should pass selectedNodeId to InfiniBandTopology', () => {
    render(<TopologyViewer nodes={mockNodes} selectedNodeId="dgx-01" />);

    fireEvent.click(screen.getByText('InfiniBand'));

    expect(screen.getByText('Selected: dgx-01')).toBeInTheDocument();
  });
});
