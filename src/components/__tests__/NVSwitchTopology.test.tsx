...
describe('NVSwitchTopology', () => {
  const createMockGPU = (id: number, health: 'OK' | 'Warning' | 'Critical' = 'OK'): GPU => ({
    id,
  ...
  describe('Instructions', () => {
    it('should show click instruction', () => {
      ...
      expect(screen.getByText(/Click on a GPU to select it/)).toBeInTheDocument();
    });
  ...
  describe('Fault Injection Button', () => {
    ...
    it('should not show inject button when onFaultInject provided but no GPU selected', () => {
      ...
      render(<NVSwitchTopology node={mockNode} onFaultInject={onFaultInject} />);
  ...
  describe('Props', () => {
    ...
    it('should accept showDataFlow prop', () => {
      render(<NVSwitchTopology node={mockNode} showDataFlow={true} dataFlowPath={[0, 7]} />);

      expect(screen.getByText(/NVSwitch Fabric Topology/)).toBeInTheDocument();
    });

    it('should accept dataFlowPath prop', () => {
      render(<NVSwitchTopology node={mockNode} showDataFlow={true} dataFlowPath={[0, 4]} />);
      ...
      const nodeWithInactiveLinks: DGXNode = {
        ...
        gpus: mockNode.gpus.map((gpu, idx) => ({
          ...
          nvlinks: gpu.nvlinks.map((link, linkIdx) => ({
            ...link,
            status: idx === 0 && linkIdx === 0 ? 'Down' as const : 'Active' as const,
          })),
        })),
      };
