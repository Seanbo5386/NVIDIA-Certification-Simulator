...
export class CmshSimulator extends BaseSimulator {
     ...
     */
    executeInteractive(input: string, _context: CommandContext): CommandResult {
        ...
        if (command === 'list') {
            return { output: this.handleList(args), exitCode: 0, prompt: this.getPrompt() };
        }

        // Handle 'show' - show details of current object
        if (command === 'show') {
            return { output: this.handleShow(), exitCode: 0, prompt: this.getPrompt() };
        }
    ...
    private handleList(args: string[]): string {
        ...
        if (this.state.currentMode === 'device' || !this.state.currentMode) {
            if (jsonFlag) {
                ...
                const jsonData = nodes.map(node => ({
                    ...
                    'ip': `10.141.0.${parseInt(node.id.split('-')[1] || '1') + 1}`,
                    'category': `dgx-${node.systemType.toLowerCase()}`,
                }));
                return JSON.stringify(jsonData, null, 2);
            }

            // Table output per spec Golden Output Reference
            let output = '| Name (key)         | Network     | IP           | Mac               | Category     |\n';
            output += '| ------------------ | ----------- | ------------ | ----------------- | ------------ |\n';
            ...
            nodes.forEach((node, idx) => {
                ...
                output += `| ${node.hostname.padEnd(18)} | internalnet | ${ip.padEnd(12)} | ${mac} | ${category.padEnd(12)} |\n`;
            });
        ...
        if (this.state.currentMode === 'softwareimage') {
            ...
            let output = '| Name (key)           | Path                           | Kernel version      | Nodes |\n';
            output += '| -------------------- | ------------------------------ | ------------------- | ----- |\n';
            output += '| baseos-image-v10     | /cm/images/baseos-image-v10    | 5.15.0-1035-nvidia  | 32    |\n';
            output += '| maintenance-image    | /cm/images/maintenance-image   | 5.15.0-1035-nvidia  | 0     |\n';
            return output;
        }
    ...
    private handleShow(): string {
        ...
        if (this.state.currentMode === 'device' && this.state.currentObject) {
            const nodes = this.getNodes();
            const node = nodes.find(n => n.hostname === this.state.currentObject || n.id === this.state.currentObject);
    ...
    execute(parsed: ParsedCommand, _context: CommandContext): CommandResult {
        ...
        if (command === 'device' || command === 'category' || command === 'softwareimage') {
        ...
        return this.createError(`cmsh: unknown command '${command}'. Run 'cmsh --help' for usage.`);
    }
}
