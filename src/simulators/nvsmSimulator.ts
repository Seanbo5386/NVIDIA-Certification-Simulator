...
export class NvsmSimulator extends BaseSimulator {
      ...
      commands: [
        {
          ...
          examples: ['nvsm', 'nvsm show health', 'nvsm show health --detailed', 'nvsm dump health'],
        },
      ],
    };
  }
   ...
   */
  private formatHealthCheck(description: string, status: 'Healthy' | 'Warning' | 'Critical'): string {
   ...
   */
  private generateHealthChecks(node: DGXNode): { checks: string[]; healthy: number; total: number } {
    const checks: string[] = [];
    let healthy = 0;
    let total = 0;

    // DIMM Memory check
    checks.push(this.formatHealthCheck('Verify installed DIMM memory sticks', 'Healthy'));
    healthy++; total++;

    // CPU cores check
    checks.push(this.formatHealthCheck('Number of logical CPU cores', 'Healthy'));
    healthy++; total++;
  ...
  private showHealth(node: DGXNode, detailed: boolean = false): string {
    ...
    const overallStatus = healthy === total ? 'Healthy' :
      checks.some(c => c.includes('Critical')) ? 'Critical' : 'Warning';
  ...
  private changeDirectory(path: string, node: DGXNode): CommandResult {
    ...
    if (path.startsWith('/')) {
      ...
      // Relative path
      newPath = this.shellState.currentPath === '/'
        ? `/${path}`
  ...
  executeInteractive(input: string, context: CommandContext): CommandResult {
    ...
    if (!node) {
      return { output: 'Error: Cannot connect to NVSM daemon. Is nvsm-core service running?', exitCode: 1 };
    }
    ...
    switch (verb) {
      ...
      case 'show':
        if (args[0] === 'health') {
          const detailed = args.includes('--detailed') || args.includes('-display');
          return { output: this.showHealth(node, detailed), exitCode: 0, prompt: this.getPrompt() };
        }
        return { output: this.showTarget(node), exitCode: 0, prompt: this.getPrompt() };

      case 'dump':
        if (args[0] === 'health') {
          return { output: this.dumpHealth(), exitCode: 0, prompt: this.getPrompt() };
        }
        return { output: 'Usage: dump health', exitCode: 1, prompt: this.getPrompt() };
      ...
      case 'help':
        return { output: this.generateHelp(), exitCode: 0, prompt: this.getPrompt() };
  ...
  execute(parsed: ParsedCommand, context: CommandContext): CommandResult {
    ...
    const node = this.getNode(context);
    if (!node) {
      return this.createError('Error: Cannot connect to NVSM daemon. Is nvsm-core service running?\nHint: Run \'sudo systemctl status nvsm-core\' to check service status.');
    }
    ...
    if (command?.startsWith('/')) {
      ...
      if (hasShow) {
        ...
        if (gpuMatch) {
          const gpuId = parseInt(gpuMatch[1]);
          if (gpuId >= 0 && gpuId < node.gpus.length) {
            return this.createSuccess(this.showGPUDetails(node.gpus[gpuId], gpuId));
          }
        }
        return this.createSuccess(this.showTarget(node));
      }
    ...
    return this.createError(`Error: Unknown command '${command}'. Type 'nvsm --help' for usage.`);
  }
