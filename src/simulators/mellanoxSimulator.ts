...
export class MellanoxSimulator extends BaseSimulator {
      ...
      commands: [
        ...
        },
        {
          ...
          examples: [
            ...
            "mlxconfig -d /dev/mst/mt4119_pciconf0 set INTERNAL_CPU_MODEL=1",
          ],
        },
        {
          name: "mlxlink",
          ...
          examples: [
            "mlxlink -d /dev/mst/mt4119_pciconf0",
            "mlxlink -d /dev/mst/mt4119_pciconf0 -c",
          ],
        },
        {
          name: "mlxcables",
        ...
        },
        {
          ...
          examples: [
            "mlxup -d /dev/mst/mt4119_pciconf0 -q",
            "mlxup -d /dev/mst/mt4119_pciconf0 --online",
          ],
        },
        {
          name: "mlxfwmanager",
          ...
          examples: [
            "mlxfwmanager",
            "mlxfwmanager --query",
            "mlxfwmanager -d /dev/mst/mt4119_pciconf0 --query",
          ],
        },
      ],
    };
  ...
  private handleMLXConfig(
    ...
    if (command === "q" || command === "query") {
      ...
      const cpuModel = isDPU
        ? (device as BlueFieldDPU).mode.internalCpuModel
        : 0;
    ...
    if (command === "s" || command === "set") {
      ...
      if (key === "INTERNAL_CPU_MODEL") {
        ...
        if (!isDPU) {
          return this.createError(
            "Error: INTERNAL_CPU_MODEL is only supported on BlueField DPUs",
          );
        }
      ...
      return this.createSuccess(
        `Applying configuration ${key}=${value}...\nDone!`,
      );
    }
  ...
  private handleMLXLink(
    ...
    if (!devicePath) {
      return this.createError(
        "Error: Device not specified. Use -d /dev/mst/<device>",
      );
    }
    ...
    if (this.hasAnyFlag(parsed, ["c"])) {
      ...
      if (port.errors.symbolErrors > 0) {
        output +=
          "\x1b[33m⚠ Warning: Symbol errors detected. Check cable quality.\x1b[0m\n";
      }
      if (port.errors.linkDowned > 0) {
        output +=
          "\x1b[31m⚠ Critical: Link has gone down. Check cable connection.\x1b[0m\n";
      }
  ...
  private handleMLXUp(
    ...
    if (imagePath) {
      ...
      output +=
        "\x1b[33m-I- Please reboot the system to load new firmware\x1b[0m\n";
  ...
  private handleMLXFwManager(
    ...
    const devicePath = this.getFlagString(parsed, ["d"]);
    const doQuery =
      this.hasAnyFlag(parsed, ["query"]) ||
      parsed.subcommands.includes("query");
    ...
    if (doQuery || (!onlineQuery && !doUpdate)) {
      ...
      output +=
        "-------------------------------------------------------------\n";
      output +=
        "  Device #     Device Type    Part Number     PSID             Firmware\n";
      output +=
        "-------------------------------------------------------------\n";
    ...
    if (onlineQuery) {
      ...
      devices.forEach((device, idx) => {
        ...
        const status =
          currentFw === availableFw ? "Up to date" : "Update available";
    ...
    return this.createError(
      "Usage: mlxfwmanager [--query|--online-query|-u] [-d <device>]",
    );
  }
}
