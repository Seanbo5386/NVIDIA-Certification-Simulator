...
export class BcmSimulator extends BaseSimulator {
  ...
  private handleBcm(parsed: ParsedCommand, context: CommandContext): CommandResult {
    // Handle --help flag
    if (this.hasAnyFlag(parsed, ['help', 'h'])) {
  ...
  private handleBcmNode(parsed: ParsedCommand, context: CommandContext): CommandResult {
    ...
    let output = '\n╔═══════════════════════════════════════════════════════════════════╗\n';
    output += '║                    BCM Node Inventory                              ║\n';
    output += '╚═══════════════════════════════════════════════════════════════════╝\n\n';

    output += 'Node ID'.padEnd(COL_NODEID) +
    ...
    nodes.forEach(node => {
      const status = node.healthStatus === 'OK' ? '\x1b[32mHealthy\x1b[0m' :
        node.healthStatus === 'Warning' ? '\x1b[33mWarning\x1b[0m' :
  ...
  // bcm ha status
  private handleBcmHA(parsed: ParsedCommand, context: CommandContext): CommandResult {
    ...
    let output = '\n╔═══════════════════════════════════════════════════════════════════╗\n';
    output += '║              Base Command Manager - High Availability              ║\n';
    output += '╚═══════════════════════════════════════════════════════════════════╝\n\n';
  ...
  // bcm job list
  private handleBcmJobList(_parsed: ParsedCommand, _context: CommandContext): CommandResult {
    ...
    let output = '\n╔═══════════════════════════════════════════════════════════════════╗\n';
    output += '║                    BCM Deployment Jobs                             ║\n';
    output += '╚═══════════════════════════════════════════════════════════════════╝\n\n';

    output += 'Job ID'.padEnd(COL_JOBID) +
      'Type'.padEnd(COL_TYPE) +
      'Status'.padEnd(COL_STATUS) +
    ...
    this.jobs.forEach(job => {
      const status = job.status === 'completed' ? '\x1b[32mcompleted\x1b[0m' :
        job.status === 'running' ? '\x1b[33mrunning\x1b[0m' :
          job.status === 'failed' ? '\x1b[31mfailed\x1b[0m' :
            'pending';

      output += job.id.toString().padEnd(COL_JOBID) +
  ...
  // bcm job logs
  private handleBcmJobLogs(parsed: ParsedCommand, _context: CommandContext): CommandResult {
    ...
    if (job.type === 'node-discovery') {
      ...
      output += '[2024-01-10 09:45:05] Reboot required for changes to take effect\n';
    }
  ...
  // bcm validate pod
  private handleBcmValidate(parsed: ParsedCommand, context: CommandContext): CommandResult {
    ...
    let output = '\n╔═══════════════════════════════════════════════════════════════════╗\n';
    output += '║              SuperPOD Configuration Validation                     ║\n';
    output += '╚═══════════════════════════════════════════════════════════════════╝\n\n';
    ...
    // Check 8: GPU Health
    const healthyGPUs = nodes.reduce((sum, n) =>
      sum + n.gpus.filter(g => g.healthStatus === 'OK').length, 0
    );
    ...
    if (healthyGPUs === totalGPUs) {
      ...
      output += '\x1b[33m⚠ SuperPOD validation completed with warnings\x1b[0m\n';
    }
  ...
  private handleCrm(parsed: ParsedCommand, context: CommandContext): CommandResult {
    const command = parsed.subcommands[0];
    if (command !== 'status') {
  ...
  private handleCrmStatus(_parsed: ParsedCommand, context: CommandContext): CommandResult {
    const cluster = this.getCluster(context);
