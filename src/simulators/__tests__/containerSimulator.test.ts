...
describe("ContainerSimulator", () => {
  ...
  describe("Metadata", () => {
    it("should return correct metadata", () => {
      ...
      expect(metadata.commands).toHaveLength(4);
  ...
  describe("Docker Commands", () => {
    ...
    describe("docker run", () => {
      ...
      it("should run container with specific GPU device", () => {
        const parsed = parse(
          "docker run --gpus device=0 nvidia/cuda:12.4.0-base nvidia-smi",
        );
        const result = simulator.execute(parsed, context);
      ...
      it("should run container with multiple GPU devices", () => {
        const parsed = parse(
          "docker run --gpus device=0,1 nvidia/cuda:12.4.0-base nvidia-smi",
        );
        const result = simulator.execute(parsed, context);
      ...
      it("should handle MIG device specification", () => {
        const parsed = parse(
          "docker run --gpus device=MIG-GPU-0 nvidia/cuda:12.4.0-base nvidia-smi",
        );
        const result = simulator.execute(parsed, context);
      ...
      it("should reject invalid GPU specification", () => {
        const parsed = parse(
          "docker run --gpus invalid nvidia/cuda:12.4.0-base",
        );
        const result = simulator.execute(parsed, context);
      ...
      it("should execute command without nvidia-smi", () => {
        const parsed = parse(
          "docker run --gpus all nvidia/cuda:12.4.0-base python train.py",
        );
        const result = simulator.execute(parsed, context);
    ...
    describe("enroot import", () => {
      it("should import docker image with manual positionalArg", () => {
        ...
        expect(result.output).toContain(
          "Importing docker://nvidia/cuda:12.4.0-base",
        );
        expect(result.output).toContain("100%");
  ...
  describe("nvidia-container-cli Commands", () => {
    describe("nvidia-container-cli info", () => {
      it("should show GPU and driver information", () => {
        const parsed = parse("nvidia-container-cli info");
        const result = simulator.execute(parsed, context);

        expect(result.exitCode).toBe(0);
        expect(result.output).toContain("NVRM version:");
        expect(result.output).toContain("CUDA version:");
        expect(result.output).toContain("535.129.03");
        expect(result.output).toContain("12.2");
      });

      it("should show GPU device details", () => {
        const parsed = parse("nvidia-container-cli info");
        const result = simulator.execute(parsed, context);

        expect(result.exitCode).toBe(0);
        expect(result.output).toContain("Device Index:");
        expect(result.output).toContain("Model:");
        expect(result.output).toContain("NVIDIA H100 80GB HBM3");
        expect(result.output).toContain("Brand:");
        expect(result.output).toContain("GPU UUID:");
      });

      it("should match scenario validation pattern nvidia|gpu|container", () => {
        const parsed = parse("nvidia-container-cli info");
        const result = simulator.execute(parsed, context);

        expect(result.exitCode).toBe(0);
        // Validation pattern: "info|driver|device"
        expect(result.output).toMatch(/NVIDIA|GPU|Device/i);
      });
    });

    describe("nvidia-container-cli list", () => {
      it("should list GPU device files", () => {
        const parsed = parse("nvidia-container-cli list");
        const result = simulator.execute(parsed, context);

        expect(result.exitCode).toBe(0);
        expect(result.output).toContain("/dev/nvidiactl");
        expect(result.output).toContain("/dev/nvidia-uvm");
        expect(result.output).toContain("/dev/nvidia0");
      });
    });

    describe("nvidia-container-cli configure", () => {
      it("should succeed with confirmation", () => {
        const parsed = parse("nvidia-container-cli configure");
        const result = simulator.execute(parsed, context);
    ...
    describe("nvidia-container-cli help", () => {
      it("should show help when no subcommand", () => {
        const parsed = parse("nvidia-container-cli");
        // Remove any subcommands since there are none
        parsed.subcommands = [];
        const result = simulator.execute(parsed, context);
    ...
    describe("nvidia-container-cli unknown command", () => {
      it("should error for unknown subcommand", () => {
        const parsed = parse("nvidia-container-cli badcmd");
        const result = simulator.execute(parsed, context);
  ...
  describe("Error Cases", () => {
    ...
    it("should handle MIG device not found", () => {
      ...
      const parsed = parse(
        "docker run --gpus device=MIG-GPU-1 nvidia/cuda:12.4.0-base nvidia-smi",
      );
      const result = simulator.execute(parsed, context);
