...
describe('NeMoSimulator', () => {
  ...
  describe('Burn-in Subcommand', () => {
    ...
    it('should monitor training stability indicators', () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.output).toContain('Monitoring: loss convergence, GPU utilization, memory stability');
    });
    ...
    it('should report GPU utilization in expected range (95-99%)', () => {
      ...
      const utilValues = Array.from(utilMatches).map(match => parseFloat(match[1]));
    ...
    it('should show decreasing loss over iterations', () => {
      ...
      const lossMatches = result.output.matchAll(/Iteration \d+\/\d+: loss=(\d+\.\d+)/g);
      const lossValues = Array.from(lossMatches).map(match => parseFloat(match[1]));
  ...
  describe('Output Format Validation', () => {
    ...
    it('should use consistent metric formatting', () => {
      ...
      const throughputMatches = result.output.matchAll(/throughput=(\d+) samples\/sec/g);
