...
export class IpmitoolSimulator extends BaseSimulator {
  ...
  private registerValidFlagsAndSubcommands(): void {
    ...
    this.registerValidSubcommands([
      'sensor', 'mc', 'chassis', 'lan', 'user', 'sdr', 'sel', 'fru', 'dcmi', 'raw',
      'power', 'event', 'pef', 'sol', 'channel', 'session', 'nm', 'sunoem',
      'kontronoem', 'picmg', 'fwum', 'firewall', 'delloem', 'shell', 'exec', 'set', 'hpm'
    ]);
  }
  ...
  /**
   * Generate realistic ipmitool help output
   */
  protected handleHelp(commandName?: string): CommandResult {
    ...
    let output = `ipmitool version 1.8.18\n\n`;
    output += `usage: ipmitool [options...] <command>\n\n`;

    output += `       -h             This help\n`;
    output += `       -V             Show version information\n`;
    output += `       -v             Verbose (can use multiple times)\n`;
    output += `       -c             Display output in comma separated format\n`;
    output += `       -d N           Specify a /dev/ipmiN device to use (default=0)\n`;
    output += `       -I intf        Interface to use\n`;
    output += `       -H hostname    Remote host name for LAN interface\n`;
    output += `       -p port        Remote RMCP port [default=623]\n`;
    output += `       -U username    Remote session username\n`;
    output += `       -f file        Read remote session password from file\n`;
    output += `       -z size        Change Size of Communication Channel\n`;
    output += `       -S sdr         Use local file for remote SDR cache\n`;
    output += `       -D tty:b[:s]   Specify the serial device, baud rate, etc.\n`;
    output += `       -4             Use only IPv4\n`;
    output += `       -6             Use only IPv6\n`;
    output += `       -a             Prompt for remote password\n`;
    output += `       -P, -E         Remote password or IPMI_PASSWORD\n`;
    output += `       -L level       Remote session privilege level [default=ADMINISTRATOR]\n`;
    output += `                      Append a '+' to use name/privilege lookup in RAKP1\n`;
    output += `       -A authtype    Force use of auth type NONE, PASSWORD, MD2, MD5 or OEM\n`;
    output += `       -t addr        Bridge request to remote target address\n`;
    output += `       -b channel     Set destination channel for bridged request\n`;
    output += `       -C ciphersuite Cipher suite to be used by lanplus interface\n`;
    output += `       -l lun         Set destination lun for raw commands\n`;
    output += `       -m address     Set local IPMB address\n`;
    output += `       -o oemtype     Setup for OEM (use 'list' to see available OEM types)\n`;
    output += `       -O seloem      Use file for OEM SEL event descriptions\n`;
    output += `       -N seconds     Specify timeout for lan/lanplus interface\n`;
    output += `       -R retry       Set the number of retries for lan/lanplus interface\n\n`;

    output += `Interfaces:\n`;
    output += `       open          Linux OpenIPMI Interface [default]\n`;
    output += `       imb           Intel IMB Interface\n`;
    output += `       lan           IPMI v1.5 LAN Interface\n`;
    output += `       lanplus       IPMI v2.0 RMCP+ LAN Interface\n\n`;

    output += `Commands:\n`;
    output += `       raw           Send a RAW IPMI request and print response\n`;
    output += `       i2c           Send an I2C Master Write-Read command\n`;
    output += `       spd           Print SPD info from remote I2C device\n`;
    output += `       lan           Configure LAN Channels\n`;
    output += `       chassis       Get chassis status and set power state\n`;
    output += `       power         Shortcut to chassis power commands\n`;
    output += `       event         Send pre-defined events to MC\n`;
    output += `       mc            Management Controller status and global enables\n`;
    output += `       sdr           Print Sensor Data Repository entries and readings\n`;
    output += `       sensor        Print detailed sensor information\n`;
    output += `       fru           Print built-in FRU and scan SDR for FRU locators\n`;
    output += `       sel           Print System Event Log (SEL)\n`;
    output += `       pef           Configure Platform Event Filtering (PEF)\n`;
    output += `       sol           Configure and connect IPMIv2.0 Serial-over-LAN\n`;
    output += `       user          Configure Management Controller users\n`;
    output += `       channel       Configure Management Controller channels\n`;
    output += `       session       Print session information\n`;
    output += `       dcmi          Data Center Management Interface\n`;
    output += `       nm            Node Manager Interface\n`;
    output += `       sunoem        OEM Commands for Sun servers\n`;
    output += `       kontronoem    OEM Commands for Kontron devices\n`;
    output += `       picmg         Run a PICMG/ATCA extended cmd\n`;
    output += `       fwum          Update IPMC using Kontron OEM Firmware Update\n`;
    output += `       firewall      Configure Firmware Firewall\n`;
    output += `       delloem       OEM Commands for Dell systems\n`;
    output += `       shell         Launch interactive IPMI shell\n`;
    output += `       exec          Run list of commands from file\n`;
    output += `       set           Set runtime variable for shell and exec\n`;
    output += `       hpm           Update HPM components using PICMG HPM.1 file\n`;
   ...
   */
  private getSensorThresholds(sensor: BMCSensor): { lnr: string; lc: string; lnc: string; unc: string; uc: string; unr: string } {
    ...
    if (name.includes('pwr') || name.includes('power') || name.includes('watts')) {
      return {
        lnr: 'na',
        lc: 'na',
        lnc: 'na',
    ...
    // Voltage sensors
    if (name.includes('volt') || name.includes('v') && sensor.unit.toLowerCase() === 'volts') {
      return {
        lnr: '0.000',
        lc: '0.000',
        lnc: '0.000',
   ...
   */
  private handleSensor(parsed: ParsedCommand, context: CommandContext): CommandResult {
    ...
    return this.createError('sensor: Missing or invalid subcommand\nUsage: ipmitool sensor [list|get|thresh]');
  }

  /**
   * Handle mc command
   */
  private handleMc(parsed: ParsedCommand, context: CommandContext): CommandResult {
    const node = this.getNode(context);
    if (!node) {
      return this.createError('Error: Unable to determine current node');
    }
    ...
    return this.createError('mc: Missing or invalid subcommand\nUsage: ipmitool mc [info|reset|guid]');
  }

  /**
   * Handle chassis command
   */
  private handleChassis(parsed: ParsedCommand, context: CommandContext): CommandResult {
    const node = this.getNode(context);
    if (!node) {
      return this.createError('Error: Unable to determine current node');
    }
    ...
    if (subsubcommand === 'status') {
      const output = `System Power         : ${node.bmc.powerState === 'On' ? 'on' : 'off'}\n` +
    ...
    if (subsubcommand === 'power') {
      ...
      if (action === 'status') {
        return this.createSuccess(`Chassis Power is ${node.bmc.powerState === 'On' ? 'on' : 'off'}`);
      } else if (['on', 'off', 'cycle', 'reset'].includes(action)) {
        return this.createSuccess(`Chassis Power Control: ${action.charAt(0).toUpperCase() + action.slice(1)}`);
      }
      return this.createError('chassis power: Missing or invalid action\nUsage: ipmitool chassis power [status|on|off|cycle|reset]');
    }

    return this.createError('chassis: Missing or invalid subcommand\nUsage: ipmitool chassis [status|power]');
  }

  /**
   * Handle lan command
   */
  private handleLan(parsed: ParsedCommand, context: CommandContext): CommandResult {
    const node = this.getNode(context);
    if (!node) {
      return this.createError('Error: Unable to determine current node');
    }
    ...
    if (subsubcommand === 'print') {
      const output = `Set in Progress         : Set Complete\n` +
    ...
    if (subsubcommand === 'set') {
      ...
      if (param && value) {
        return this.createSuccess(`Setting LAN parameter "${param}" to "${value}"`);
      }
      return this.createError('lan set: Missing parameter or value\nUsage: ipmitool lan set <channel> <param> <value>');
    }

    return this.createError('lan: Missing or invalid subcommand\nUsage: ipmitool lan [print|set] <channel>');
  }

  /**
   * Handle user command
   */
  private handleUser(parsed: ParsedCommand, _context: CommandContext): CommandResult {
    const subsubcommand = parsed.subcommands[1];

    if (subsubcommand === 'list') {
      const output = `ID  Name             Callin  Link Auth  IPMI Msg   Channel Priv Limit\n` +
    ...
    if (subsubcommand === 'set') {
      ...
      if (action && userId) {
        return this.createSuccess(`Set User ${action} command successful (user ${userId})`);
      }
      return this.createError('user set: Missing action or user ID\nUsage: ipmitool user set <action> <user_id>');
    }

    return this.createError('user: Missing or invalid subcommand\nUsage: ipmitool user [list|set|enable|disable]');
  }

  /**
   * Handle sdr command
   */
  private handleSdr(parsed: ParsedCommand, context: CommandContext): CommandResult {
    const node = this.getNode(context);
    if (!node) {
      return this.createError('Error: Unable to determine current node');
    }
    ...
    return this.createError('sdr: Missing or invalid subcommand\nUsage: ipmitool sdr [list|info|type|get]');
  }
   ...
   */
  private handleSel(parsed: ParsedCommand, context: CommandContext): CommandResult {
    const subsubcommand = parsed.subcommands[1];
    const node = this.getNode(context);
      ...
      node.gpus.forEach((gpu, gpuIdx) => {
        ...
        if (gpu.xidErrors && gpu.xidErrors.length > 0) {
          gpu.xidErrors.forEach(xid => {
            ...
            if (isExtended) {
              entries.push(`   ${hexId} | ${timeStr} | GPU #${gpuIdx} | XID ${xid.code} - ${xid.description} | Asserted`);
            } else {
              entries.push(`   ${hexId} | ${timeStr} | GPU #${gpuIdx} | XID ${xid.code} | ${xid.description}`);
            }
            recordId++;
          });
        }
        ...
        if (gpu.temperature > 80) {
          ...
          if (isExtended) {
            entries.push(`   ${hexId} | ${timeStr} | GPU #${gpuIdx} | Temperature - Upper Critical going high (${gpu.temperature}C) | Asserted`);
          } else {
            entries.push(`   ${hexId} | ${timeStr} | GPU #${gpuIdx} | Upper Critical | Temperature (${gpu.temperature}C) exceeded threshold`);
          }
          recordId++;
        }
        ...
        if (gpu.eccErrors && gpu.eccErrors.doubleBit > 0) {
          ...
          if (isExtended) {
            entries.push(`   ${hexId} | ${timeStr} | GPU #${gpuIdx} | Memory - Uncorrectable ECC error detected | Asserted`);
          } else {
            entries.push(`   ${hexId} | ${timeStr} | GPU #${gpuIdx} | Memory | Uncorrectable ECC error detected`);
          }
          recordId++;
        }
      });
    ...
    if (subsubcommand === 'clear') {
      return this.createSuccess('Clearing SEL. Please allow a few seconds to erase.\n');
    }
    ...
    return this.createError('sel: Missing or invalid subcommand\nUsage: ipmitool sel [list|elist|clear|info]');
  }

  /**
   * Handle fru command
   */
  private handleFru(parsed: ParsedCommand, context: CommandContext): CommandResult {
    const node = this.getNode(context);
    if (!node) {
      return this.createError('Error: Unable to determine current node');
    }
    ...
    return this.createError('fru: Missing or invalid subcommand\nUsage: ipmitool fru [print|list]');
  }
   ...
   */
  private handleDcmi(parsed: ParsedCommand, context: CommandContext): CommandResult {
    const node = this.getNode(context);
    if (!node) {
      return this.createError('Error: Unable to determine current node');
    }
      ...
      if (subsubsubcommand === 'set_limit') {
        ...
        if (setLimitIdx !== -1 && allArgs[setLimitIdx + 1]) {
          ...
          if (!isNaN(directValue)) {
            ...
            return this.createSuccess(`Power Limit set to ${directValue} Watts\n`);
          }
        }

        return this.createError('dcmi power set_limit: Missing limit value\nUsage: ipmitool dcmi power set_limit limit <watts>');
      }

      return this.createError('dcmi power: Missing subcommand\nUsage: ipmitool dcmi power [reading|get_limit|set_limit]');
    }
    ...
    return this.createError('dcmi: Missing or invalid subcommand\nUsage: ipmitool dcmi [power|discover|sensors]');
  }
   ...
   */
  private handleRaw(parsed: ParsedCommand, _context: CommandContext): CommandResult {
    ...
    try {
      ...
      if (netfn === 0x3c) {
        ...
        if (cmd === 0x80) {
          ...
          return this.createSuccess(` ${lowByte.toString(16).padStart(2, '0').toUpperCase()} ${highByte.toString(16).padStart(2, '0').toUpperCase()}`);
        }
      }
