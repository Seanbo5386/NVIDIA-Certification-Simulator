...
export class ContainerSimulator extends BaseSimulator {
      ...
      commands: [
        {
          ...
          examples: ['docker ps', 'docker images', 'docker run --gpus all nvidia/cuda:12.4.0-base nvidia-smi'],
        },
        {
          name: 'ngc',
          description: 'NVIDIA GPU Cloud CLI',
          usage: 'ngc [COMMAND] [OPTIONS]',
          examples: ['ngc config set', 'ngc registry image list', 'ngc registry model list'],
        },
        {
          name: 'enroot',
          description: 'Unprivileged container runtime',
          usage: 'enroot [COMMAND] [OPTIONS]',
          examples: ['enroot import docker://nvidia/cuda:12.4.0-base', 'enroot list', 'enroot start container-name'],
        },
      ],
    };
  }
  ...
  // Docker commands
  private handleDocker(parsed: ParsedCommand, context: CommandContext): CommandResult {
    // Handle --help flag
    if (this.hasAnyFlag(parsed, ['help', 'h'])) {
      ...
      output += 'Run \'docker COMMAND --help\' for more information on a command.\n';
    ...
    if (command === 'run') {
      ...
      if (!gpuSpec) {
        return this.createError('Error: GPU specification missing. Use --gpus <spec>', 125);
      }
      ...
      if (gpuSpec.startsWith('device=')) {
        ...
        if (devices[0].startsWith('MIG-GPU')) {
          ...
          if (migDeviceMatch) {
            ...
            if (!gpu || !gpu.migMode) {
              return this.createError('Error: MIG device not found or MIG mode not enabled', 125);
            }
      ...
      return this.createError('Error: Invalid GPU specification. Use "all" or "device=0,1,..."', 125);
    }
    ...
    if (command === 'ps') {
      ...
      let output = 'CONTAINER ID'.padEnd(COL_CONTAINERID) +
      ...
      this.containers.forEach(container => {
        output += container.id.padEnd(COL_CONTAINERID) +
    ...
    if (command === 'images') {
      ...
      let output = 'REPOSITORY'.padEnd(COL_REPOSITORY) +
        'TAG'.padEnd(COL_TAG) +
      ...
      this.images.forEach(image => {
        output += image.repository.padEnd(COL_REPOSITORY) +
    ...
    if (command === 'pull') {
      ...
      if (!this.images.find(i => i.repository === repo && i.tag === (tag || 'latest'))) {
        this.images.push({
          repository: repo,
          tag: tag || 'latest',
  ...
  // NGC CLI commands
  private handleNGC(parsed: ParsedCommand, _context: CommandContext): CommandResult {
    const command = parsed.subcommands[0];
    ...
    if (command === 'config') {
      ...
      if (subCommand === 'set') {
        ...
        return this.createSuccess('NGC CLI configuration set successfully.\nAPI key saved.');
      }

      if (subCommand === 'current') {
        if (!this.ngcConfigured) {
          return this.createError('NGC CLI not configured. Run "ngc config set" first.');
        }

        return this.createSuccess('Current NGC Configuration:\n  API Key: ****...****\n  Org: nvidia\n  Team: default');
      }
    ...
    if (!this.ngcConfigured) {
      return this.createError('Error: NGC CLI not configured. Run "ngc config set" first.');
    }
    ...
    if (command === 'registry') {
      ...
      if (subCommand === 'image') {
        ...
        if (action === 'list') {
          ...
          output += 'NAME                                     TAG          SIZE       LAST MODIFIED\n';
          output += '-----------------------------------------------------------------------\n';
        ...
        if (action === 'info') {
          const image = parsed.positionalArgs[0] || 'nvcr.io/nvidia/pytorch:24.01-py3';
      ...
      if (subCommand === 'model') {
        ...
        if (action === 'list') {
          ...
          output += '---------------------------------------------------------------------\n';
          output += 'nvidia/nemo-megatron-gpt-20b        20B parameter GPT model\n';
          output += 'nvidia/llama-2-70b                  Meta Llama 2 70B\n';
          output += 'nvidia/stable-diffusion-xl          Text-to-image model\n';
          output += 'nvidia/resnet50                     Image classification\n';

          return this.createSuccess(output);
        }
      }

      return this.createError('Usage: ngc registry <image|model> <list|pull|info> [name]');
    }
  ...
  // Enroot commands
  private handleEnroot(parsed: ParsedCommand, _context: CommandContext): CommandResult {
    const command = parsed.subcommands[0];
    ...
    if (command === 'create') {
      ...
      const containerName = image.replace(/[/:]/g, '-') + '-' + Math.random().toString(36).substring(7);

      return this.createSuccess(`Creating container from ${image}...\nContainer created: ${containerName}`);
    }

    if (command === 'list') {
      return this.createSuccess('Available enroot containers:\n  pytorch-24.01-py3-abc123\n  tensorflow-24.01-def456');
    }
    ...
    if (command === 'start') {
      ...
      return this.createSuccess(`Starting container ${container}...\nContainer started successfully.`);
    }

    return this.createError('Usage: enroot <import|create|list|start> [options]');
  }
}
